...
layuiXtree.prototype.dataBind = function (d) {
 ...
 if (d!=null&&d.length > 0) {
     for (i in d) {
         ...
         if(sum==0){
			//_this._domStr += '<div class="layui-xtree-root">';
			_this._domStr += '<ul class="layui-xtree-root">'
		 }else{
			//_this._domStr += '<div class="layui-xtree-item">';
			_this._domStr += '<li class="layui-xtree-item">';
		}
         if(d[i].isopen!=null){
		...
		_this._domStr += '<li class="layui-xtree-item">'
         if (d[i].children!=null&&d[i].children.length > 0){
			//_this._domStr += '<ul class="layui-xtree-root"><li>'
             _this._domStr += '<i class="layui-icon layui-xtree-icon " style="margin-left:-15px;" data-xtree="' + (_this._isopen ? '1' : '0') + '">' + (_this._isopen ? _this._iconOpen : _this._iconClose) + '</i>';
         _this._domStr += '<i class="layui-icon layui-icon-export" style="padding-right:2px"></i>';
         }else if(d[i].type=='menu'){
             _this._domStr += '<i class="layui-icon layui-icon-export" style="padding-right:2px"></i>';
         }else {
         ...
         if(d[i].request_mode){
         	_this._domStr += '<span class="'+d[i].request_mode+'">'+d[i].request_mode+'</span>';
         } else if(d[i].type=='api'){
			_this._domStr += '<span class="GET">GET</span>';
		 }
         _this._domStr += '<span data-id="'+d[i].id+'" data-type="'+d[i].type+'" '+xtree_isend+' class="xtree-title">'+d[i].title+'</span>';
         _this._domStr += '<i class="layui-icon layui-icon-more xtree-menu-more" data-id="'+d[i].id+'" data-type="'+d[i].type+'"></i>';
 		_this._domStr += '</li>'
...
layuiXtree.prototype.Rendering = function () {
 ...
 var xtree_titles = document.getElementsByClassName('xtree-title');
 var xtree_icons_export = document.getElementsByClassName('layui-icon-export');

 for (var i = 0; i < xtree_items.length; i++) {
     if (xtree_items[i].parentNode == _this._container)
         xtree_items[i].style.padding = '5px 0 0 15px';
     else {
         xtree_items[i].style.padding = '5px 0 0 20px';
 ...
 for (var i = 0; i < xtree_icons.length; i++) {
     ...
     //xtree_icons[i].style.margin = "0 5px 0 0";
     xtree_icons[i].style.fontSize = "14px";
  ...
  for (var i = 0; i < xtree_titles.length; i++) {
     xtree_titles[i].onclick = function () {
         var xtree_chi = this.parentNode.childNodes;
         xtree_chi[0].click();
     }
 }
 
 for (var i = 0; i < xtree_icons_export.length; i++) {
     xtree_icons_export[i].onclick = function () {
         var xtree_chi = this.parentNode.childNodes;
         xtree_chi[0].click();
     }
 }
